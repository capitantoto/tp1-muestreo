...
* Cuando expresemos sumatorias sobre la característica objetivo, omitiremos los subíndices. Así, escribiremos $\sum_{U} y$ en lugar de $\sum{k \in U} y_{k}$. 
...
$$
\begin{align}
\widehat{Var}(\hat{\Delta}) &= N^2 \sum\limits_{h=1}^{H}\left[W_{h}^{2}\left(\frac{1}{n_{h}}-\frac{1}{N_{h}}\right)\left(s^{2}_{t_{1},h}+s^{2}_{t_{0},h}\right)\right] \\ 
&- 2 N^{2} \sum\limits_{h=1}^{H}\left[W_{h}^{2}\left(\frac{n_{c,h}}{n^{2}_{h}}-\frac{1}{N_{h}}\right)s_{t_{1},t_{0}c,h}\right] 
\end{align}
$$
...
$$
\widehat{Var}(\hat{\Delta}_{c}) = N^2 \sum\limits_{h=1}^{H}\left[W_{h}^{2}\left(\frac{1}{n_{c,h}}-\frac{1}{N_{h}}\right)\left(s^{2}_{t_{1},c,h}+s^{2}_{t_{0},c,h}- 2 s_{t_{1},t_{0}c,h}\right)\right]
$$
