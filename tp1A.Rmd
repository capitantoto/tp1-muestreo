...
library(knitr)
opts_chunk$set(fig.width=12, fig.height=9, message=FALSE)
```
...
rug(POmarco, col= 'blue', ticksize = 0.1, lwd = 1)
```
...
En el trabajo de los autores, se proveen fórmulas para calcular la diferencia entre dos estimadores de Horvitz-Thompson para los totales de dos características (o una misma observada en dos momentos en el tiempo, como es nuestro caso), bajo *diseños sin estratificar*. Para poder aplicarlas a nuestro caso de uso, deberemos hacer algunas modificaciones.

Antes de proponer las alternativas, algunas definiciones.

* Sean $s_{t}$ y $s_{t-1}$ las muestras tomadas en t y t-1, respectivamente.
* Sean $N_{t} = N_{t-1} = N$ los tamaños (idénticos) de las muestras $s_{t}$ y $s_{t-1}$. En nuestro caso, $N=248$.
* Similarmente, sean $n_{h,t} = n_{h,(t-1)} = n_{h}$ los tamaños de muestra por estrato para todo estrato h=1,...,H (en nuestro caso, $H = 5$).
* Sean $s_{c}$ el "solapamiento" entre las muestras $s_{t}$ y $s_{t-1}$, $n_{h,c}$ el tamaño muestral por estrato compartido entre ambas, y $N_{c}$ el tamaño de $s_{c}$. En nuestro caso particular, para todo h < H, $n_{h,c} = 0.8 n_{h}$, y $n_{H,c} = n_{H}$

#### Estimador a partir de los totales muestrales

$\hat{\Delta} = \hat{Y_{t}} - \hat{Y_{t-1}}$

donde $\hat{Y_{t}}=\sum\limits_{k \in s_{t}} 

donde 


* Sea $n_{t-1}$ el tamaño de la muestra 
