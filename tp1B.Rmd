...
#### Dominio "Norte"
```{r resultados_norte, echo=FALSE}
print("CVs alcanzados versus esperados:")
tablaCVNorte
print("Resumen por estrato:")
tablaEstratosNorte
print("Lista de estratos inestables:")
malosEstratos(tablaEstratosNorte)
```

#### Dominio "centro"
```{r resultados_centro, echo=FALSE}
print("CVs alcanzados versus esperados:")
tablaCVCentro
print("Resumen por estrato:")
tablaEstratosCentro
...
Para ejecutar la optimización por algoritmo genético, debemos pasar, al igual que a `bethel`, los resltados de `buildstrata` junto con una tabla con las precisiones por variable objetivo y dominio:
...
comparacion_corridas a<- rbind(pruebas[[1]]$resumen,
			       pruebas[[2]]$resumen,
			       pruebas[[3]]$resumen,
			       pruebas[[4]]$resumen,
			       pruebas[[5]]$resumen,
...
De esta comparación, lo primera que llama la atención, es que $N$ es básicamente idéntico para las seis pruebas: la razón entre el menor y mayor valor es de $5565/5549=1.0029$, o un 0,3%.
...
![Convergencia Dominio Norte](prueba2/plotdom1.pdf)

#### Convergencia Dominio Centro
![Convergencia Dominio Centro](prueba2/plotdom2.pdf)

#### Convergencia Dominio Sur
![Convergencia Dominio Sur](prueba2/plotdom3.pdf)
...
Para finalizar, hacemos un histograma de la cantidad de microestratos que agrupa cada nuevo estrato, para hacernos una idea de cuánto hemos "comprimido" la estratificación original:

```{r grafico_cant_estratos_agrupados}
plot(hist(sumario$cantEstratosAgrupados))
```
...
muestra <- selectSample(pruebas[[2]]$nuevoMarco,pruebas[[2]]$solu$aggr_strata)
```
...
2. si $h(k) = h(l) = h \wedge k \neq l$, entonces $\pi_{kl}=\frac{n_{h}}{N_{h}}\frac{n_{h}-1}{N_{h}-1}
3. si $h(k)\neq h(l)$, entonces $\pi_{kl}=\pi_{k}\pi_{l}$

Dado que el caso 1 se puede observar en el vector de $\pi_{k}$'s, y el caso 2 se puede aproximar como $\pi_{k}^{2}$ cuando $n_{h}$ y $N_{h}$ son lo suficientemente grandes, consideramos que proveer el vector con las 40 $\pi_{k}$ y las 40x40 $\pi_{kl}$ del caso 3 son una aproximación suficiente a los requerimientos de este ejercicio. Eso, y en algún momento tengo que dejar de escribir y comenzar a editar este trabajo.

```{r pik_y_pikl}
aggr_strata <- pruebas[[2]]$solu$aggr_strata
